...
本章重点难点：了解列表和元组的基本使用方法、了解字典的基本使用方法
...
本章重点难点：如何打开和保存文件，如何总和使用列表和字典，如何使用json文件。
...
模块的定义，绝对导入和相对导入，模块重命名， __name__和__main__。
...
派生类构造函数的定义、构造规则以及创建派生类对象时的调用顺序。
...
__repr__()和__str__()方法，__format__()方法，__hash__()方法，比较运算符，__new__()和__del__()方法，__add__()方法

11. 多重继承                                                                

多重继承的概念和声明；多重继承派生类构造规则和调用顺序。
